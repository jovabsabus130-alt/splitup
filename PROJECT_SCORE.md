...
| **10** | Frontend | **Frontend deployment** | No | 0.2 | `frontend/vercel.json`, `frontend/netlify.toml` | Single Page Application (SPA) routing rewrites, asset caching headers, and automated build scripts. |
...
| **16** | Backend & System Design | **Server-side error handling** | **Yes** | 0.2 | `src/middleware/errorHandler.js`, `src/utils/serverErrorHandling.js`, `src/routes/` | Comprehensive error handling architecture: custom error hierarchy (`AppError`, `NotFoundError`, `ForbiddenError`, `ConflictError`), 4-arity Express middleware, operational vs programmer error isolation, Prisma/Zod/JWT mappers, and safe structured JSON responses. |
| **17** | Backend & System Design | **Middleware** | **Yes** | 0.2 | `src/middleware/auth.js`, `src/middleware/sanitize.js`, `cors()`, `express.json()` | Custom JWT auth middleware and input sanitization middleware guarding the Express request pipeline. |
...
| **19** | Backend & System Design | **Backend deployment** | No | 0.2 | `render.yaml`, `Procfile`, `package.json` | Render Blueprint configuration (`render.yaml`) and Procfile web process for automated cloud deployment. |
| **20** | NoSQL (Mongo) | **Schema modeling (Mongo)** | **Yes** | 0.2 | `src/models/RawExpenseLog.js` | Mongoose schema definition with subdocuments, timestamps, and compound indexing for AI parse audits. |
| **21** | NoSQL (Mongo) | **CRUD operations (Mongo)** | **Yes** | 0.2 | `src/routes/aiExpense.js`, `src/models/RawExpenseLog.js` | Create, Read, and Query operations for unstructured AI prompt outputs and performance metrics. |
| **22** | NoSQL (Mongo) | **Embedding vs referencing** | No | 0.2 | `src/models/RawExpenseLog.js` | Embedded `metrics` & `predictions` subdocuments for high-locality reads; referenced `groupId` & `userId` foreign IDs. |
| **23** | NoSQL (Mongo) | **Aggregation pipelines** | No | 0.2 | `src/models/RawExpenseLog.js` | Multi-stage aggregation pipelines (`$match`, `$group`, `$project`, `$sort`, `$limit`) for AI latency & category stats. |
| **24** | NoSQL (Mongo) | **Indexing for performance (Mongo)** | No | 0.2 | `src/models/RawExpenseLog.js` | Compound indexing on `{ groupId: 1, createdAt: -1 }` for rapid chronological query resolution. |
...
| **36** | Auth & Security | **Input sanitization & injection awareness** | No | 0.2 | `src/middleware/sanitize.js`, `src/routes/auth.js` | Middleware stripping malicious HTML (XSS prevention) and stripping `$`/`.` keys (NoSQL injection defense). |
...
| **46** | AI App Eng | **Token & cost monitoring** | No | 0.3 | `src/models/RawExpenseLog.js` | Logs prompt tokens, completion tokens, and latency to MongoDB for cost tracking. |
...
| **50** | Engineering Practices | **Writing unit tests** | No | 0.3 | `src/__tests__/` (20 unit tests across 3 suites) | Automated unit test suite using native Node test runner (`node:test`) testing algorithms, security, and services. |
| **51** | Engineering Practices | **Containerization with Docker** | No | 0.5 | `Dockerfile`, `frontend/Dockerfile`, `docker-compose.yml` | Multi-stage Docker build containerizing Express backend, NGINX frontend, PostgreSQL, and Redis. |
...
| **53** | System & Integration | **Caching with Redis** | No | 0.4 | `src/services/redisCache.js`, `src/__tests__/servicesAndSecurity.test.js` | TTL caching layer with pattern-based cache invalidation for group balance calculations. |
| **54** | System & Integration | **WebSocket / real-time communication** | No | 0.5 | `src/services/websocketService.js`, `src/__tests__/servicesAndSecurity.test.js` | Group channel pub/sub WebSocket engine broadcasting live expense and settlement updates. |
| **55** | System & Integration | **Scheduled jobs / cron** | No | 0.3 | `src/services/cronService.js`, `src/__tests__/servicesAndSecurity.test.js` | Background cron scheduler periodically purging expired OTP verification records and maintaining ledgers. |
| **56** | System & Integration | **Server-side rendering** | No | 0.5 | `src/services/ssrService.js`, `src/__tests__/servicesAndSecurity.test.js` | Server-Side Rendering generating dynamic HTML and OpenGraph metadata tags for rich social link previews. |
